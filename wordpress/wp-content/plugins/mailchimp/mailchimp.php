...
function mailchimp_sf_get_interest_categories( $list_id, $new_list, $update_option = true ) {
	...
	if ( is_wp_error( $igs ) || !is_array( $igs ) || empty( $igs['categories'] ) || !is_array( $igs['categories'] ) ) {
		if ( $update_option ) {
			update_option( 'mc_interest_groups', [] );
		}
		return [];
	}

	foreach ( $igs['categories'] as $key => $ig ) {
		$groups = $api->get( "lists/{$list_id}/interest-categories/{$ig['id']}/interests", 60 );

		// Normalize groups response
		$interests = ( is_wp_error( $groups ) || !is_array( $groups ) || empty( $groups['interests'] ) )
			? []
			: $groups['interests'];

		$igs['categories'][ $key ]['groups'] = $interests;

		// turn them all on by default for a new list
		if ( $new_list ) {
			update_option( 'mc_show_interest_groups_' . $ig['id'], 'on' );
		}
	}

	$categories = $igs['categories'];

	if ( $update_option ) {
		update_option( 'mc_interest_groups', $categories );
	}
