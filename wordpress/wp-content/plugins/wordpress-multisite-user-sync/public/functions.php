...
if ( ! function_exists( 'wmus_user_sync_unsync' ) ) {
    ...
    function wmus_user_sync_unsync( $profileuser ) {
        ...
        if ( $sync_type == 'manual' ) {
            ...
            if ( is_super_admin() ) {
                ?>
                    <h2><?php esc_html_e( 'WordPress Multisite User Sync/Unsync', 'wordpress-multisite-user-sync' ); ?></h2>
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td>
                                    <fieldset>
                                        <label>
                                            <input type="radio" name="wmus_sync_unsync" value="1" checked="checked" /><?php esc_html_e( 'Sync', 'wordpress-multisite-user-sync' ); ?>
                                        </label>
                                        <label>
                                            <input type="radio" name="wmus_sync_unsync" value="0" /><?php esc_html_e( 'Unsync', 'wordpress-multisite-user-sync' ); ?>
                                        </label>
                                    </fieldset>
                            ...
                            <tr>
                                <th><label><?php esc_html_e( 'Sites', 'wordpress-multisite-user-sync' ); ?></label></th>
                                <td>
                                            ...
                                            if ( $sites != null ) {
                                                foreach ( $sites as $key => $value ) {
                                                    ...
                                                    if ( ( $value->blog_id != get_current_blog_id() ) || ( is_network_admin() ) ) {
                                                        ?>
                                                            <label><input name="wmus_blogs[]" type="checkbox" value="<?php echo esc_attr( $value->blog_id ); ?>"<?php echo esc_attr( $checked ); ?>><?php echo esc_attr( $value->domain ); echo esc_attr( $value->path ); echo ' ('.esc_attr($blog_details->blogname).')'; ?></label><br>
                                                        <?php
                                                    }
                                                }
                                            }
